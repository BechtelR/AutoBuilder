...
| 40 | Director stateless -- config object recreated per invocation | All state in DB via DatabaseSessionService; personality in `user:` scope, seeded from config on first login, evolvable; Director acts as COO + personal assistant (Phase 5) | 2026-02-16 |
...
| 45 | No new ADK scope for project config | Project config as DB entity (project table with CRUD/permissions/versioning); existing 4 ADK scopes sufficient: `app:` = global, `user:` = CEO prefs + Director personality, session = per-session, `temp:` = scratch (Phase 5) | 2026-02-16 |
