...
| ✓ | Project-scope agent definitions rejected if `type: custom` (`tool_role` ceiling validation deferred to Phase 7a) | NFR-4b |
...
### Scope Summary
Supervision hierarchy: Director operates as stateless root_agent (recreated per invocation), delegates to PM via transfer_to_agent, PM escalates back. Director processes its own queue (Director Queue) during work sessions — reads pending PM escalations, resolves or forwards to CEO queue. Director formation via Settings conversation — three structured artifacts in user: scope. Director "Main" project as permanent default chat session. PM loop in sequential mode — validates inter-batch reasoning with tools + callbacks; Phase 8b adds parallel execution. Deterministic safety mechanisms: checkpoint_project (after_agent_callback on DeliverablePipeline), verify_batch_completion (after_agent_callback). Director supervision callbacks (before/after_agent_callback). Hard limits cascade (CEO → Director → PM → Workers) — retry_budget and cost_ceiling enforced in Phase 5b; concurrency limit enforcement deferred to Phase 8b. CEO queue gateway routes (GET /ceo/queue, PATCH /ceo/queue/{id}) with resolved approval → session state writeback. Chat gateway routes (POST/GET /chat/{session_id}/messages). Cross-session bridge (app:/user: state + Redis Streams). State key authorization via tier prefixes (director:, pm:, worker:) enforced by EventPublisher ACL (Decision #58). Context recreation end-to-end validation (persist → fresh session → reassemble in degraded mode).
...
| ✓ | PM loop (sequential mode): PM reasons correctly between batches — queries results, decides next batch composition, maintains coherent state across batch boundaries (2+ deliverables); Phase 8b adds parallel execution | PR-10 |
...
## Phase 8b: Parallel Execution & Isolation `M`

**Goal**: Upgrade the sequential autonomous loop to parallel batch execution with filesystem isolation and concurrency control. Multiple deliverables execute concurrently in isolated git worktrees.
**Status**: PLANNED
**Prerequisites**: Phase 8a (autonomous loop operational with sequential execution)

### Scope Summary
PM batch execution upgraded from sequential (Phase 8a) to parallel via `select_ready_batch()` returning multiple deliverables dispatched concurrently through ParallelAgent composition. Git worktree isolation: create worktree per deliverable (branch derived from current HEAD), merge on completion in dependency order, cleanup on failure. Deterministic merge conflict resolution strategy (sequential merge in dependency order; conflict → PM escalation). Parallel worker state key namespacing (`worker:{deliverable_id}:*`) to prevent state collision between concurrent workers sharing the PM session. Concurrency limit enforcement (max parallel pipelines, configurable and cascaded from project-level limits — deferred from Phase 5b). Human-in-the-loop proactive intervention API (`POST /workflows/{id}/intervene`) for CEO-initiated pause/modify at batch boundaries.

### Completion Contract

| Status | Contract Item | PRD |
|--------|--------------|-----|
| | PM constructs dependency-aware parallel batches and dispatches via ParallelAgent | PR-10 |
| | Git worktrees provide filesystem isolation — one worktree per deliverable, merged on completion | PR-9 |
| | Worktree merges execute in dependency order; merge conflicts escalate to PM for resolution | PR-9 |
| | Parallel workers use namespaced state keys (`worker:{deliverable_id}:*`) — no state collision | PR-10 |
| | Concurrency limits enforced: max parallel pipelines capped and cascaded from project-level limits | PR-15, NFR-2 |
| | Can intervene (pause/modify) at batch boundary via proactive API — CEO initiates, not just system escalates | PR-3, PR-9 |
...
**Status**: PLANNED
**Prerequisites**: Phase 8a (autonomous loop producing completed sessions)
...
| | Gateway workflow routes operational (list, select, run — wiring WorkflowRegistry from Phase 7a) | PR-4 |
| | CLI can launch a workflow, stream events, and query status | PR-36 |
| | OpenTelemetry traces visible for pipeline execution | PR-35 |
| | **Full MVP validated end-to-end**: brief -> decompose -> execute -> verify -> complete | PR-1, PR-10, PR-22 |
