"""LLM Router — maps model roles to LiteLLM model strings with fallback chains."""
...
# Maps agent names to their model roles for before_model_callback routing
AGENT_MODEL_ROLES: dict[str, ModelRole] = {
    "echo_agent": ModelRole.FAST,
}
...
class LlmRouter:
    """Routes model roles to LiteLLM model strings with fallback resolution."""

    def __init__(self, defaults: dict[ModelRole, str]) -> None:
    ...
    def from_settings(cls, settings: Settings) -> LlmRouter:
        ...
        return cls(
            defaults={
                ModelRole.CODE: settings.default_code_model,
                ModelRole.PLAN: settings.default_plan_model,
                ModelRole.REVIEW: settings.default_review_model,
                ModelRole.FAST: settings.default_fast_model,
            }
        )

    def select_model(self, model_role: ModelRole, user_override: str | None = None) -> str:
        """Select model string for a model role. User override wins if provided."""
        if user_override is not None:
            return user_override
        return self._defaults[model_role]
...
def create_model_override_callback(
    ...
    def before_model_callback(
        ...
        model = router.select_model(model_role, user_override=user_override)
        llm_request.model = model
        logger.debug(
            "Model override",
            extra={"agent": agent_name, "model_role": model_role, "model": model},
        )
        return None
